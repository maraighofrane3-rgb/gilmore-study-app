...
serve(async (req) => {
  ...
  try {
    ...
    // sinon on retombe sur l'heure du serveur (moins fiable, mais mieux que rien)
    let localHour = null
    try {
    ...
    if (!rawContent) {
      throw new Error(`Groq returned empty content. finish_reason: ${choice?.finish_reason}`)
    }
