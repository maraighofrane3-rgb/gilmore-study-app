    ...
    const fetchData = async () => {
      ...
      const [tasksResult, focusResult, goalsResult, streakResult, monthlyFocusResult] =
        await Promise.all([
          ...
          // NOTE: assumes a `duration_minutes` column on focus_sessions.
          // Rename below to match your actual schema (e.g. duration_seconds / 60).
          supabase
            .from('focus_sessions')
      ...
      // Aggregate raw sessions into one { date, minutes } entry per day
      // for the monthly bar chart.
      const minutesByDay = {};
      (monthlyFocusResult.data ?? []).forEach((session) => {
        const day = session.created_at.split('T')[0];
        minutesByDay[day] = (minutesByDay[day] || 0) + (session.duration_minutes || 0);
      });
      setStudyLog(
